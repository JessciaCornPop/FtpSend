/* DO NOT EDIT THIS FILE - it is machine generated */
#include <jni.h>
/* Header for class com_example_myapplication_MainActivity */

#ifndef _Included_com_example_myapplication_MainActivity
#define _Included_com_example_myapplication_MainActivity
#ifdef __cplusplus
extern "C" {
#endif
/*
 * Class:     com_example_myapplication_MainActivity
 * Method:    ftpUploadFromJNI
 * Signature: ()Ljava/lang/String;
 */
JNIEXPORT jstring JNICALL Java_com_example_myapplication_MainActivity_FtpUploadFromJNI
  (JNIEnv *, jobject);
JNIEXPORT jstring JNICALL Java_com_example_myapplication_MainActivity_FtpDownloadFromJNI
(JNIEnv *, jobject);

#ifdef __cplusplus
}
#endif
#endif
